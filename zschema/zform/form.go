...
import "github.com/denghuo98/zzframe/zconsts"
...
type PageReq struct {
	...
	PerPage    int  `json:"pageSize" example:"1" d:"10" v:"min:1|max:5000#每页数量最小值不能低于1|最大值不能大于5000" dc:"每页数量"`
...
// CalPage 计算分页偏移量, 输入不合法则返回默认值
func CalPage(page, perPage int) (newPage, newPerPage int, offset int) {
	if page <= 0 {
		newPage = zconsts.DefaultPage
	} else {
		newPage = page
	}
	if perPage <= 0 {
		newPerPage = zconsts.DefaultPageSize
	} else {
		newPerPage = perPage
	}
