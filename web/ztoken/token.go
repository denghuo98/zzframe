...
var (
	config          = &webSchema.TokenConfig{}
	errorLogin      = gerror.New("登录身份已失效，请重新登录！")
	errorMultiLogin = gerror.New("账号已在其他地方登录，如非本人操作请及时修改登录密码！")
)

func SetConfig(c *webSchema.TokenConfig) {
	config = c
}

func GetConfig() *webSchema.TokenConfig {
	return config
}

// GetAuthKey 认证key
func GetAuthKey(token string) string {
	return gmd5.MustEncryptString("zzframe" + token)
}

// GetTokenKey 令牌缓存key
func GetTokenKey(appName, authKey string) string {
	return fmt.Sprintf("%v:%v:%v", zconsts.CacheToken, appName, authKey)
}

// GetBindKey 令牌身份绑定key
func GetBindKey(appName string, userId int64) string {
	return fmt.Sprintf("%v:%v:%v", zconsts.CacheTokenBind, appName, userId)
}
...
// parseToken 解析jwt令牌
func parseToken(ctx g.Ctx, header string) (*Claims, error) {
...
func ParseLoginUser(r *ghttp.Request) (user *webSchema.Identity, err error) {
	var (
		ctx    = r.Context()
	...
	// 检查token是否存在
	tk, err := zcache.Instance().Get(ctx, tokenKey)
	if err != nil {
	...
	if !config.MultiLogin {
		origin, err := zcache.Instance().Get(ctx, bindKey)
		if err != nil {
	...
	refreshToken := func() {
		...
		if err = zcache.Instance().Set(ctx, tokenKey, token, duration); err != nil {
			return
		}

		if err = zcache.Instance().Set(ctx, bindKey, tokenKey, duration); err != nil {
			return
		}
	}
	...
	g.Go(ctx, func(ctx g.Ctx) {
		...
		g.Log().Errorf(ctx, "refreshToken err:%+v", exception)
	})
...
func Logout(r *ghttp.Request) (err error) {
	...
	var (
		...
		authKey = GetAuthKey(header)
		// 登录token
		tokenKey = GetTokenKey("web", authKey)
		// 身份绑定
		bindKey = GetBindKey("web", claims.Id)
	)

	// 删除token
	if _, err = zcache.Instance().Remove(ctx, tokenKey); err != nil {
		return
	}

	if !config.MultiLogin {
		if _, err = zcache.Instance().Remove(ctx, bindKey); err != nil {
			return
		}
	}
	return
...
// Login 登录
func Login(ctx g.Ctx, user *webSchema.Identity) (string, int64, error) {
	claims := Claims{
		user,
		jwt.RegisteredClaims{},
	}
